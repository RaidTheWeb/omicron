...
int main(int argc, const char **argv) {
    ...
    OUtils::SlabAllocator allocator = OUtils::SlabAllocator();
    char *ptr = (char *)allocator.alloc(69);
    snprintf(ptr, 69, "Hello World!");
    printf("%s %lu\n", ptr, sizeof(Transform));
    allocator.free(ptr);

    OUtils::ResolutionTable table = OUtils::ResolutionTable(4096);
    OUtils::HandlePointer<OScene::GameObject> handle = OUtils::HandlePointer<OScene::GameObject>(&table, table.bind(obj));

    printf("%lu\n", handle.handle);

    printf("%f %f %f\n", handle->transform.getposition().x, handle->transform.getposition().y, handle->transform.getposition().z);
