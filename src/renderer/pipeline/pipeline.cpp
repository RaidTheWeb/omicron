...
void PBRPipeline::execute(ORenderer::Stream *stream) {
    ...
    data.model = glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f)); // rotate Z
    ...
    memcpy(ubomap.mapped[ORenderer::context->getlatency()], &data, sizeof(struct ubo));
