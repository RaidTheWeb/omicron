        ...
        for (auto it = stream->cmd.begin(); it != stream->cmd.end(); it++) {
            switch (it->type) {
                ...
                case ORenderer::Stream::OP_COMMITRESOURCES: {
                    ...
                    // TODO: Cache this stuff so we aren't redoing the entire descriptor set every frame
                    ...
                    vkUpdateDescriptorSets(this->dev, stream->mappings.size(), wds, 0, NULL);
                    stream->mappings.clear(); // clear mappings
                    stream->tempmem.freeto(marker);
